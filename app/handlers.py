...
async def get_library(message: Message):
    await message.answer_photo(
        ...
        caption="⚫Обучение и развитие сотрудников – один из важнейших элементов"
...
@router.message(F.text == "Этикет")
async def get_etiket(message: Message):
    formatted_text = (
        "\n\n\n\n\n"  # Добавляем большие пробелы
        "Давай я расскажу тебе о «внутренней кухне» Beeline Кыргызстан.\n"
        "\n"
        "Мы ценим свободу сотрудников, но, тем не менее, придерживаемся правил, которые делают "
        "нашу общую работу в офисе комфортной:\n"
        "\n"
        "• Мы здороваемся, вне зависимости от того, коллега это или сотрудник из другой дирекции;\n"
        "• Мы уважаем чужой труд, поэтому моем за собой посуду после обеда на общей кухне, а также "
        "поддерживаем чистоту в офисе. Это касается и румов после собраний - задвинуть за собой стулья, "
        "стереть записи с доски;\n"
        "• Мы поддерживаем коллег, если нас просят поучаствовать в опросе или прийти на Курултай "
        "(встреча с топ-менеджментом Компании, когда мы подводим итоги квартала), или поучаствовать в "
        "BeeStyle или «Перезагрузка» (ежеквартальный субботник, когда мы приводим в порядок офис);\n"
        "• Мы вместе веселимся, потому что нет ничего лучше, чем отпраздновать общий успех с теми, "
        "кто внес оклад в его достижение.\n"
        "\n\n\n\n\n"  # Ещё большие пробелы
    )
